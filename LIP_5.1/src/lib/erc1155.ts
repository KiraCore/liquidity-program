...
const abi = [
  ...
  },
  {
    "inputs": [
      {
        "internalType": "contract IKexFarm",
        "name": "farmer",
        "type": "address"
      }
    ],
    "name": "setFarmerAddress",
    "outputs": [
      {
        "internalType": "bool",
        "name": "",
...
export const create = (address: string, provider: any) => {
  ...
  // READ
  const balanceOf = (owner: string, id: number) => contract.balanceOf(owner, id).then((res: any) => res.toNumber());

  const tokenSupply = (card: number) => contract.tokenSupply(card);
